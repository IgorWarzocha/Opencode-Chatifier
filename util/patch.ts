...
export namespace Patch {
  ...
  export function deriveNewContentsFromChunks(filePath: string, chunks: UpdateFileChunk[]): ApplyPatchFileUpdate {
    ...
    try {
      originalContent = fsSync.readFileSync(filePath, "utf-8")
    } catch (error) {
  ...
  function computeReplacements(
    ...
    for (const chunk of chunks) {
      ...
      if (chunk.change_context) {
        ...
        // Set lineIndex to contextIdx (not +1) so old_lines can include the context line
        lineIndex = contextIdx
      }

      // Handle pure addition/replacement (no old lines specified)
      if (chunk.old_lines.length === 0) {
        if (chunk.change_context && contextIdx !== -1) {
          // No old_lines but has context: REPLACE the context line with new_lines
          // This handles the common case: @@ target line / +replacement line
      ...
      // Try to match old lines in the file (starting from context position)
